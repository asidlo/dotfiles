function lps --description "$USER - Copies password to clipboard from lastpass"
  ...
  if command -s lpass >/dev/null
    ...
    if not test $status -eq 0
      ...
      set -l pass (lpass ls -l | eval "fzf $FZF_DEFAULT_OPTS --ansi --header='[lastpass:copy]'" | grep -o "id: [0-9]*" | cut -d':' -f2 | string trim)
