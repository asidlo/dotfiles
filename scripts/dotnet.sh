...
# The silent NuGet credential provider is installed by its own top-level step
# (scripts/devcontainer-credprovider.sh, invoked from install.sh) so that every
# environment gets it, not just the full installs that run this script. Run that
# script directly if you need it without a full install.
